...
def film_in_category(category_id:int)->pd.DataFrame:
    ...
    return pd.read_sql_query(zap, con=connection)
...
def number_films_in_category(category_id:int)->pd.DataFrame:
    ...
    zap     =   f"""select 
                        c.name as category, count(c.name)
                    from 
                        category c
                    join 
...
def client_from_city(city:str)->pd.DataFrame:
    ...
    zap =   f"""select 
                    city.city, c.first_name, c.last_name
                from 
                    city
                join 
                    address on address.city_id = city.city_id
                join 
                    customer c on c.address_id = address.address_id
                where 
                    city.city = '{city}'
                order by
...
def client_by_sum_length(sum_min:Union[int,float])->pd.DataFrame:
    ...
    zap =   f"""select c.first_name, c.last_name, sum(f.length) as sum
                from 
                    film as f
                join 
                    inventory i on i.film_id = f.film_id
...
def category_statistic_length(name:str)->pd.DataFrame:
    ...
    zap=    f"""select 
                    c.name as category, avg(f.length), sum(f.length), min(f.length), max(f.length)
                from 
                    film f
                join 
